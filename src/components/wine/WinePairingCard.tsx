...
const WINES: Record<Phase, { headline: string; mood: string; items: WineItem[] }> = {
  menstrual: {
    ...
    items: [
      { emoji: '🍷', name: 'Malbec',            reason: 'Plush tannins and dark fruit soothe without overwhelming a sensitive system' },
      { emoji: '🍷', name: 'Merlot',             reason: 'Soft, rounded, and easy — the comfort food of red wines' },
      { emoji: '🍷', name: 'Grenache',           reason: 'Low tannin and gently spiced, kind to cramp-prone days' },
      { emoji: '🫖', name: 'Mulled wine',        reason: 'Warming spices like cinnamon and clove actively ease inflammation' },
      { emoji: '🍷', name: 'Sangiovese',         reason: 'Medium body with cherry brightness — lifts mood without taxing energy' },
  ...
  follicular: {
    ...
    items: [
      ...
      { emoji: '🥂', name: 'Prosecco',           reason: 'Celebratory bubbles suit your most socially confident phase' },
      { emoji: '🥂', name: 'Pinot Grigio',       reason: 'Crisp and clean — lets your natural energy do the talking' },
      { emoji: '🍷', name: 'Pinot Noir',         reason: 'Light-bodied red with earthy elegance for evenings winding down' },
      { emoji: '🥂', name: 'Grüner Veltliner',  reason: 'Peppery and mineral — great for trying something new this phase' },
      { emoji: '🥂', name: 'Cava',               reason: 'Spanish fizz with toasty depth — ideal for spontaneous plans' },
    ],
  },
  ovulatory: {
    ...
    items: [
      { emoji: '🥂', name: 'Champagne',          reason: 'You\'re magnetic right now — effervescent wines match the energy' },
      { emoji: '🌸', name: 'Dry Rosé',           reason: 'Strawberry and peach notes suit your warmth and open social window' },
      { emoji: '🥂', name: 'Viognier',           reason: 'Floral, full-bodied white that matches peak oestrogen\'s lushness' },
      { emoji: '🍷', name: 'Tempranillo',        reason: 'Cherry-rich with subtle spice — bold enough for your boldest phase' },
      { emoji: '🥂', name: 'White Burgundy',     reason: 'Chardonnay at its most elegant — earthy, creamy, complex' },
      { emoji: '🍷', name: 'Zinfandel',          reason: 'Fruit-forward and high-spirited — built for your social peak' },
    ],
  },
  luteal: {
    ...
    items: [
      ...
      { emoji: '🥂', name: 'Off-dry Riesling',   reason: 'A touch of sweetness steadies mood swings and sugar cravings' },
      { emoji: '🍷', name: 'Barbera',            reason: 'Low tannin, bright acidity — gentle on a system running low' },
      { emoji: '🥂', name: 'Chenin Blanc',       reason: 'Honeyed and soft — pairs beautifully with comfort food cravings' },
      { emoji: '🍷', name: 'Dolcetto',           reason: 'Bitter-cherry finish and low acid ease the luteal fog' },
  ...
  unknown: {
    ...
    items: [
      ...
      { emoji: '🥂', name: 'Prosecco',           reason: 'Lower alcohol than Champagne — a gentle celebratory choice' },
      { emoji: '🍷', name: 'Grenache',           reason: 'Soft tannins and warm fruit that work regardless of where you are' },
...
export function WinePairingCard({ cycles, prediction, bare = false }: Props) {
  ...
  const handleShare = async () => {
    ...
    if (typeof navigator.share === 'function') {
      try {
        await navigator.share({ title: 'Wine Pairings', text });
  ...
  if (bare) {
    return (
      <div className="flex flex-col">
        {/* Header */}
        <div className="px-5 pt-3 pb-3 flex items-start justify-between gap-3" style={{ borderBottom: '1px solid var(--color-peat-light)' }}>
          <div>
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="px-5 pt-5 pb-4 flex items-start justify-between gap-3" style={{ borderBottom: '1px solid var(--color-peat-light)' }}>
        <div>
          <p className="text-sm font-semibold" style={{ color: labelColor }}>Wine pairings</p>
          <p className="text-xs mt-0.5" style={{ color: style.text }}>{headline}</p>
        </div>
